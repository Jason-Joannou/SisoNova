...
from models.users import User, UserProfile, VerifyUser, UserUpdate, CleanUser
from models.business import BusinessProfile, UpdateBusinessProfile, BusinessProfileCollection
...
async def create_business_profile(mongo_client: MongoDBClient, user_business_profile_collection: BusinessProfileCollection) -> str:
    """Create a new business profile in the database"""
    try:
        async with mongo_client.get_db(secrets.mongo_db_database_name) as db:
            result = await db["business_profiles"].insert_one(user_business_profile_collection.model_dump())
            return str(result.inserted_id)
    except Exception as e:
...
async def get_user_profile(mongo_client: MongoDBClient, email: str) -> UserProfile | None:
    ...
    try:
        async with mongo_client.get_db(secrets.mongo_db_database_name) as db:
            ...
            preferred_business_profile = await db["business_profiles"].find_one({"email": email, "company_name": user_data["preferred_business_profile"]}, {"_id": 0})
            user_profile = UserProfile(
                email=user_data["email"],
                preferred_business_profile=user_data["preferred_business_profile"],
                business_profile=preferred_business_profile)
...
async def get_user_business_profile_company_names(mongo_client: MongoDBClient, email: str) -> List[str] | None:
    try:
        async with mongo_client.get_db(secrets.mongo_db_database_name) as db:
            business_profiles = await db["business_profiles"].find({"email": email}, {"_id": 0}) # Exclude _id field, 1 would include it
            return [bp["company_name"] for bp in business_profiles]
    except Exception as e:
        print(f"Error retrieving user information: {e}")
        return None
    
async def get_user_business_profiles(mongo_client: MongoDBClient, email: str) -> List[BusinessProfile] | None:
    try:
        async with mongo_client.get_db(secrets.mongo_db_database_name) as db:
            business_profiles = await db["business_profiles"].find_one({"email": email}, {"_id": 0}) # Exclude _id field, 1 would include it
...
async def get_business_profile_with_company_name(mongo_client: MongoDBClient, email: str, company_name: str) -> BusinessProfile | None:
    try:
        async with mongo_client.get_db(secrets.mongo_db_database_name) as db:
            business_profiles = await db["business_profiles"].find_one({"email": email, "company_name": company_name}, {"_id": 0}) # Exclude _id field, 1 would include it
