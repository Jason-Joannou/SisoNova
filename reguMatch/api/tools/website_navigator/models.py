...
class DuckDuckGoRequest(BaseModel):
    query: str = Field(
        ..., description="Search query (e.g., 'South Africa fishing regulations')"
    )
    num_results: int = Field(
        10, description="Number of results to return (default: 10)"
    )
...
class OpenWebsiteRequest(BaseModel):
    ...
    wait_seconds: Optional[int] = Field(
        3, description="Seconds to wait for page load (default: 4)"
    )
    default_timeout: Optional[int] = Field(
        30000, description="Default timeout for page load (default: 30000)"
    )
