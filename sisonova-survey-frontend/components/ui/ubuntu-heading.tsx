...
export function UbuntuHeading({ children, className }: UbuntuHeadingProps) {
  return (
    <div className={cn("relative flex items-center justify-center mb-8", className)}>
      ...
      <div className="relative bg-background px-4 z-10">
        <div className="flex items-center">
          <div className="w-3 h-3 rounded-full bg-ubuntu-red mr-2" />
          <h2 className="text-3xl font-bold">{children}</h2>
          <div className="w-3 h-3 rounded-full bg-ubuntu-green ml-2" />
        </div>
      </div>
    </div>
  );
